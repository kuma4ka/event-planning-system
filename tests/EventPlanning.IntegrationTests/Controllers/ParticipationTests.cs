...
public class ParticipationTests(CustomWebApplicationFactory factory) : IClassFixture<CustomWebApplicationFactory>
{
    ...
    public async Task JoinEvent_ShouldSucceed_WhenUserIsAuthenticated()
    {
        ...
        var joinResponse = await _client.PostAsync($"/events/join/{eventId}", new FormUrlEncodedContent(
        ...
        {
            var context = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
            var evt = await context.Events.Include(e => e.Guests).AsNoTracking()
    ...
    public async Task LeaveEvent_ShouldSucceed_WhenUserIsJoined()
    {
        ...
        using (var scope = factory.Services.CreateScope())
        {
            var context = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
            var organizer =
                await context.Users.AsNoTracking().FirstOrDefaultAsync(u => u.Email == "organizer@test.com");
            ...
            var evt = new Domain.Entities.Event(
                ...
                Domain.Enums.EventType.NetworkingEvent, organizer!.Id, venue!.Id);
